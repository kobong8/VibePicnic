...
#### 설정 패널 (`i`)
애니메이션 실행 중 `i` 키를 누르면 화면 오른쪽 상단에 인터랙티브 설정 패널이 열립니다. 패널이 떠 있는 동안에도 애니메이션은 뒤에서 계속 재생되며, 값 변경은 실시간으로 적용되어 저장 전에 효과를 미리 확인할 수 있습니다.
...
조정 가능한 항목: `Theme`, `Density`, `Wind`, `Speed`, `ASCII`, `Ground`. 패널 상단에는 현재 파티클 개수도 함께 표시됩니다 (예: `─ Settings ······· · 76p ─`).
